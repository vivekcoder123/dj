...
if(isset($_POST['submit'])){
	...
	if(isset($_POST['album_link'])){
		$album_link=trim($_POST['album_link']);
	}else{
		$album_link=trim("");
	}

	if(isset($_POST['video_link1'])){
		$video_link1=trim($_POST['video_link1']);
	}else{
		$video_link1=trim("");
	}

	if(isset($_POST['video_link2'])){
		$video_link2=trim($_POST['video_link2']);
	}else{
		$video_link2=trim("");
	}

	if(isset($_POST['video_link3'])){
	...
	if(isset($_FILES['image1']) && strlen($_FILES['image1']['name'])>0){
	...
	if(isset($_FILES['image2']) && strlen($_FILES['image2']['name'])>0){
	...
	if(isset($_FILES['image3']) && strlen($_FILES['image3']['name'])>0){
	...
	if(isset($_FILES['image4']) && strlen($_FILES['image4']['name'])>0){
	...
	if(isset($_FILES['image5']) && strlen($_FILES['image5']['name'])>0){
	...
	if(isset($_FILES['image6']) && strlen($_FILES['image6']['name'])>0){
	...
	if(isset($_FILES['image7']) && strlen($_FILES['image7']['name'])>0){
	...
	if(isset($_FILES['image8']) && strlen($_FILES['image8']['name'])>0){
	...
	if(isset($_FILES['image9']) && strlen($_FILES['image9']['name'])>0){
...
$sql = "INSERT INTO artist (name,description,slug,image,gallery1,gallery2,gallery3,gallery4,gallery5,gallery6,gallery7,gallery8,gallery9,category_id,album_link,video_link1,video_link2,video_link3,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
$pdo->prepare($sql)->execute([$name,$description,$slug,$image,$gallery1,$gallery2,$gallery3,$gallery4,$gallery5,$gallery6,$gallery7,$gallery8,$gallery9,$category_id,$album_link,$video_link1,$video_link2,$video_link3,date('y-m-d')]);
...
<!--market updates updates-->
    <h2 class="text-center">Create Artist</h2>
     <br>
	 <div class="market-updates">
	 	...
	 	<form action="" method="post" enctype="multipart/form-data">
	 		...
	 		<div class="form-group">
	 			...
	 			<textarea name="album_link" id="album_link" cols="100" rows="2" 
	 			class="form-control" placeholder="Paste artist embedded album link"></textarea>
	 		</div>
	 		<div class="form-group">
	 			<label for="video_link1">Artist Video Link 1</label>
	 			<textarea placeholder="Paste artist embedded video link 1" 
	 			name="video_link1" id="video_link1" class="form-control" cols="100" rows="2"></textarea>
	 		</div>
	 		<div class="form-group">
	 			<label for="video_link2">Artist Video Link 2</label>
	 			<textarea placeholder="Paste artist embedded video link 2" 
	 			name="video_link2" id="video_link2" class="form-control" cols="100" rows="2"></textarea>
	 		</div>
	 		<div class="form-group">
	 			<label for="video_link3">Artist Video Link 3</label>
	 			<textarea placeholder="Paste artist embedded video link 3" 
	 			name="video_link3" id="video_link3" class="form-control" cols="100" rows="2"></textarea>
	 		</div>
